...
from tqdm.auto import tqdm as original_tqdm  # Import the original tqdm function
...
from src.util.logging_util import get_visualization_image
...
if "__main__" == __name__:
    ...
    # Plot in inverse log space
    parser.add_argument(
        "--inverse_log_plot",
        action="store_true",
        help="Render in inverse log space.",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="Number of guided denoising steps. Default: 0 (no guided denoising).",
    )
    parser.add_argument(
        "--vis_alignment",
        type=str,
    ...
    inverse_log_plot = args.inverse_log_plot
    color_map = args.color_map
    seed = args.seed
    guided_steps = args.guided_steps
    vis_alignment = args.vis_alignment

    # TODO: Take this as argument later
    max_depth = 250
    ...
    output_dir_color_guided = os.path.join(output_dir, "depth_colored_guided")
    output_dir_tif = os.path.join(output_dir, "depth_bw")
    output_dir_npy = os.path.join(output_dir, "depth_npy")
    os.makedirs(output_dir_color, exist_ok=True)
    os.makedirs(output_dir_color_guided, exist_ok=True)
    ...
    # TODO: Remove the torch.no_grad() context manager 
    # to use the model for guided denoising
    ...
    with torch.no_grad():
        for batch in tqdm(
            ...
            pipe_out = pipe(
                ...
                batch_size=0,
                color_map=color_map,
                guided_diffusion_steps=guided_steps,
                inverse_log_plot=inverse_log_plot,
            ...
            # Denormalize depth_pred and bring it to real scale
            if "absolute" == vis_alignment:
                depth_pred_denorm = depth_pred * max_depth
            elif "inverse_log" == vis_alignment:
                maxd = max_depth
                norm_min = np.log(1/(1e-6 + maxd))
                norm_max = np.log(1/1e-6)
                log_disp = depth_pred * (norm_max - norm_min) + norm_min
                disp = np.exp(log_disp)
                depth_pred_denorm = 1 / disp
            else:
                depth_pred_denorm = depth_pred
                print(f"No scaling method found for {vis_alignment}")
            ...
            img_vis = get_visualization_image(rgb_int, depth_raw, depth_pred_denorm)
            # remove .npy from pred_basename
            pred_basename = pred_basename[:-4]
            save_to_all = os.path.join(scene_dir, "vis_all_"+pred_basename+".png")
            Image.fromarray(img_vis).save(save_to_all)
