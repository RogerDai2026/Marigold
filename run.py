...
if "__main__" == __name__:
    ...
    logging.info(
        ...
        f"inverse_log_plot = {inverse_log_plot}, "
    )

    # -------------------- Inference and saving --------------------
    # TODO: Potentially cleaner code for later:
    # def conditional_no_grad(condition):
        # return torch.no_grad() if condition else contextlib.nullcontext()
    # with conditional_no_grad(guided_steps == 0):
    
        
    if (guided_steps > 0):
        logging.info(f"Guided denoising steps: {guided_steps}")
    # with torch.no_grad():
    os.makedirs(output_dir, exist_ok=True)
    ...
    for rgb_path in tqdm(rgb_filename_list, desc="Estimating depth", leave=True):
        ...
        if (guided_steps > 0):
            colored_save_path = os.path.join(
                output_dir_color_guided, f"{pred_name_base}_colored.png"
            )
